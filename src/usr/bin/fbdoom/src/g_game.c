...
boolean         timingdemo;             // if _true, exit with report on completion 
...
boolean         netgame;                // only _true if packets are broadcast 
...
boolean         precache = _true;        // if _true, load all graphics at start 

boolean         testcontrols = _false;    // Invoked by setup to test controls
...
{ 
    ...
    mousex = mousey = 0;
    sendpause = sendsave = paused = _false;
...
{ 
...
#endif 
	if (HU_Responder (ev)) 
	    return _true;	// chat ate the event 
	if (ST_Responder (ev)) 
	    return _true;	// status window ate it 
	if (AM_Responder (ev)) 
	    return _true;	// automap ate it 
    } 
	 
    if (gamestate == GS_FINALE) 
    { 
	if (F_Responder (ev)) 
	    return _true;	// finale ate the event 
    } 
	...
	{ 
        ...
        {
	    gamekeydown[ev->data1] = _true; 
        }

	return _true;    // eat key down events 
 
      case ev_keyup: 
	if (ev->data1 <NUMKEYS) 
	    gamekeydown[ev->data1] = _false; 
	return _false;   // always let key up events filter down 
		 
      case ev_mouse: 
        SetMouseButtons(ev->data1);
	mousex = ev->data2*(mouseSensitivity+5)/10; 
	mousey = ev->data3*(mouseSensitivity+5)/10; 
	return _true;    // eat events 
...
{ 
    ...
    p->usedown = p->attackdown = _true;	// don't do anything immediately 
    p->playerstate = PST_LIVE;       
    p->health = deh_initial_health;     // Use dehacked value
    p->readyweapon = p->pendingweapon = wp_pistol; 
    p->weaponowned[wp_fist] = _true; 
    p->weaponowned[wp_pistol] = _true; 
...
//
// G_CheckSpot  
// Returns _false if the player cannot be respawned
...
{ 
    ...
    if (secretexit) 
	players[consoleplayer].didsecret = _true; 
  ...
  int		map )
{
    ...
    */
	...
	respawnmonsters = _true;
    else
	respawnmonsters = _false;
    ...
    usergame = _true;                // will be set _false if a demo
    paused = _false;
    demoplayback = _false;
    automapactive = _false;
    viewactive = _true;
...
= Returns _true if a new demo loop action will take place 
=================== 
*/ 
