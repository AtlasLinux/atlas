...
// If "floatok" _true, move would be ok
...
boolean PIT_CheckLine (line_t* ld)
{
    ...
    // the given line.
    // If this should not be allowed, return _false.
    ...
    if (!ld->backsector)
	return _false;		// one sided line
		
    if (!(tmthing->flags & MF_MISSILE) )
    {
	if ( ld->flags & ML_BLOCKING )
	    return _false;	// explicitly blocking everything

	if ( !tmthing->player && ld->flags & ML_BLOCKMONSTERS )
	    return _false;	// block monsters only
    }
...
boolean PIT_CheckThing (mobj_t* thing)
{
	...
	if (tmthing->z > thing->z + thing->height)
	    return _true;		// overhead
	if (tmthing->z+tmthing->height < thing->z)
	    return _true;		// underneath
  ...
  fixed_t	y )
{
	...
	return _false;		// solid wall or thing
    
    if ( !(thing->flags & MF_NOCLIP) )
    {
	if (tmceilingz - tmfloorz < thing->height)
	    return _false;	// doesn't fit

	floatok = _true;
	
	if ( !(thing->flags&MF_TELEPORT) 
	     &&tmceilingz - thing->z < thing->height)
	    return _false;	// mobj must lower itself to fit

	if ( !(thing->flags&MF_TELEPORT)
	     && tmfloorz - thing->z > 24*FRACUNIT )
	    return _false;	// too big a step up

	if ( !(thing->flags&(MF_DROPOFF|MF_FLOAT))
	     && tmfloorz - tmdropoffz > 24*FRACUNIT )
	    return _false;	// don't stand over a dropoff
    }
...
PTR_AimTraverse (intercept_t* in)
{
	...
	return _true;			// shot continues
    }
    
    // shoot a thing
    th = in->d.thing;
    if (th == shootthing)
	return _true;			// can't shoot self
    
    if (!(th->flags&MF_SHOOTABLE))
	return _true;			// corpse or something
	...
	return _true;			// shot over the thing

    thingbottomslope = FixedDiv (th->z - shootz, dist);

    if (thingbottomslope > topslope)
	return _true;			// shot under the thing
    ...
    return _false;			// don't go any farther
}
...
boolean	PTR_UseTraverse (intercept_t* in)
{
    ...
    //	return _false;		// don't use back side
...
//
// If anything doesn't fit anymore, _true will be returned.
// If crunch is _true, they will take damage
//  as they are being crushed.
// If Crunch is _false, you should set the sector height back
