...
void M_LoadGame (int choice)
{
    if (netgame)
    {
	M_StartMessage(DEH_String(LOADNET),NULL,_false);
	return;
    }
...
void M_SaveGame (int choice)
{
    if (!usergame)
    {
	M_StartMessage(DEH_String(SAVEDEAD),NULL,_false);
	return;
    }
...
void M_QuickSave(void)
{
    ...
    M_StartMessage(tempstring,M_QuickSaveResponse,_true);
}
...
void M_QuickLoad(void)
{
    if (netgame)
    {
	M_StartMessage(DEH_String(QLOADNET),NULL,_false);
	return;
    }
	
    if (quickSaveSlot < 0)
    {
	M_StartMessage(DEH_String(QSAVESPOT),NULL,_false);
	return;
    }
    DEH_snprintf(tempstring, 80, QLPROMPT, savegamestrings[quickSaveSlot]);
    M_StartMessage(tempstring,M_QuickLoadResponse,_true);
}
...
void M_NewGame(int choice)
{
    ...
    {
	M_StartMessage(DEH_String(NEWGAME),NULL,_false);
	return;
    }
...
void M_ChooseSkill(int choice)
{
    if (choice == nightmare)
    {
	M_StartMessage(DEH_String(NIGHTMARE),M_VerifyNightmare,_true);
	return;
    }
...
void M_Episode(int choice)
{
    if ( (gamemode == shareware)
	 && choice)
    {
	M_StartMessage(DEH_String(SWSTRING),NULL,_false);
...
void M_EndGame(int choice)
{
    ...
    if (netgame)
    {
	M_StartMessage(DEH_String(NETEND),NULL,_false);
	return;
    }
	
    M_StartMessage(DEH_String(ENDGAME),M_EndGameResponse,_true);
}
...
void M_QuitDOOM(int choice)
{
    ...
    M_StartMessage(endstring,M_QuitResponse,_true);
}
